...
public class BloomFilter {
	...
	public static void main(String[] args) throws ClassNotFoundException {
		...
		// Prepare the bloom filter with random words from wordList, and add in
		// randomWordList.
		...
		for (int i = 0; i < 4000; i++) {
			...
			final boolean answerFromBloomFilter = (bitSet.get(h1.getHashValue(word))
					&& bitSet.get(h2.getHashValue(word)) && bitSet.get(h3.getHashValue(word))
					&& bitSet.get(h4.getHashValue(word)));
			...
			if (answerFromBloomFilter) {
				System.out.println(
						"Actual Answer with less (< 100%) Probability : [" + randomWordList.contains(word) + "]");
			} else {
				System.out.println(
						"Actual Answer will be FALSE with 100 % Probability : [" + randomWordList.contains(word) + "]");
			}
